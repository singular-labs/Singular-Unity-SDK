...
static void handleDeviceAttributionData(NSDictionary* deviceAttributionInfo) {
    NSString *deviceAttributionInfoJsonString = dictionaryToJson(deviceAttributionInfo);
    sendSdkMessage("SingularDeviceAttributionCallback", deviceAttributionInfoJsonString);
}
...
extern "C" {
    ...
    bool StartSingularSession_(const char* configString){
        ...
        singularConfig.sdidReceivedHandler = ^(NSString *result) {
            sendSdkMessage("SingularSdidReceived", result);
        };
