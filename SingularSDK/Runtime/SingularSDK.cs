...
#region IAP
        ...
        bool isRestored)
    {
        if (order == null)
        {
            SingularUnityLogger.LogDebug("order is null, ignoring IAP event.");
            return;
        }
    ...
    public static void InAppPurchase(IEnumerable<Product> products, Dictionary<string, object> attributes, bool isRestored = false) {
        InAppPurchase(EVENT_NAME_IAP, products, attributes, isRestored);
    }
    ...
    public static void InAppPurchase(Product product, Dictionary<string, object> attributes, bool isRestored = false) {
        InAppPurchase(EVENT_NAME_IAP, product, attributes, isRestored);
    }
    ...
    {
        if (Application.isEditor) {
            SingularUnityLogger.LogDebug("running in editor, ignoring IAP event.");
            return;
        }
        
        if (product == null) {
            SingularUnityLogger.LogDebug("product is null, ignoring IAP event.");
            return;
        }
...
#endif // SINGULAR_SDK_IAP_ENABLED__IAP_4
#if SINGULAR_SDK_IAP_ENABLED__IAP_5
        // logic adapted from IAP4's `Product.hasReceipt`.
        hasReceipt = !string.IsNullOrEmpty(order.Info.TransactionID) && !string.IsNullOrEmpty(order.Info.Receipt);
#endif // SINGULAR_SDK_IAP_ENABLED__IAP_5
    ...
    private const string androidNativeMethodName_CustomRevenue = "customRevenue";
        ...
        public static void Revenue(string currency, double amount, Dictionary<string, object> attributes)
        {
            ...
            try
            {
                ...
                if (jniSingularUnityBridge != null)
                {
                    jniSingularUnityBridge.CallStatic("revenueWithAttributes", currency, 
                        Utilities.ToCultureInvariantString(amount), attributesAsJsonString);
                }
            #endif
            }
            catch (Exception)
        ...
        public static void CustomRevenue(string eventName, string currency, double amount, Dictionary<string, object> attributes)
        {
            ...
            try
            {
                ...
                if (jniSingularUnityBridge != null)
                {
                    jniSingularUnityBridge.CallStatic("customRevenueWithAttributes", eventName, 
                        currency, Utilities.ToCultureInvariantString(amount), attributesAsJsonString);
                }
            #endif
            }
            catch (Exception)
